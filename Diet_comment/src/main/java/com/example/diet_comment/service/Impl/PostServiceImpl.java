...
//        // 3. 构建最终的帖子查询条件
//        LambdaQueryWrapper<Post> postQuery = new LambdaQueryWrapper<>();
//        postQuery.like(Post::getTitle, keyword)
//                .or().like(Post::getContent, keyword);
//
//        if (!CollectionUtils.isEmpty(userIds)) {
//            postQuery.or(qw -> qw.in(Post::getUserId, userIds));
//        }
//
//        if (!CollectionUtils.isEmpty(shopIds)) {
//            postQuery.or(qw -> qw.in(Post::getShopId, shopIds));
//        }
//
//        List<Post> posts = this.list(postQuery);
//
...
//
//        // 一次性查询所有相关的用户和店铺信息
//        Map<Integer, UserDTO> userDTOMap = Collections.emptyMap();
//        if (!CollectionUtils.isEmpty(postUserIds)) {
//            userDTOMap = userService.listByIds(postUserIds).stream()
//                    .map(user -> new UserDTO().fromUser(user))
//                    .collect(Collectors.toMap(UserDTO::getId, userDTO -> userDTO));
//        }
//
//        Map<Integer, Shop> shopMap = Collections.emptyMap();
//        if (!CollectionUtils.isEmpty(postShopIds)) {
//            shopMap = shopService.listByIds(postShopIds).stream()
//                    .collect(Collectors.toMap(Shop::getId, shop -> shop));
//        }
//
//        // 遍历帖子列表，设置 user 和 shop
//        Map<Integer, UserDTO> finalUserDTOMap = userDTOMap;
//        Map<Integer, Shop> finalShopMap = shopMap;
//        posts.forEach(post -> {
//            post.setUser(finalUserDTOMap.get(post.getUserId()));
//            post.setShop(finalShopMap.get(post.getShopId()));
//        });



        return postMapper.searchWithUserAndShop(keyword);    }
