    ...
    <div class="post">
      ...
      <span class="replyImg">
        <span v-for="(image, index) in replyPreviewImages" :key="image" class="previewImage">
          ...
          <img :src="image" @click="openPreview(replyPreviewImages, index)" :alt="帖子图片" />
        </span>
        <div>
...
<script>
import { updatePosts } from '@/api/post'
...
export default {
  ...
  async created() {
    ...
    this.shopName = this.post.shop.name
    if (this.post.imgurls === null) {
      this.replyPreviewImages = []
      this.ImagesFiles = []
    } else {
      this.replyPreviewImages = this.post.imgurls
      await this.loadImagesFromUrls(this.post.imgurls)
    }
  },
  beforeUnmount() {
  ...
  methods: {
        ...
        const files = await Promise.all(
          urls.map(async (url, idx) => {
            try {
              ...
              if (!res.ok) {
                throw new Error(`图片加载失败：${res.status} ${res.statusText}`)
              }
              // 转换为Blob（二进制数据）
              ...
              console.error(`第${idx}张图片加载失败：`, url, err)
              return null
            }
          }),
        )
        this.ImagesFiles = files.filter((file) => file !== null)
      } catch (e) {
        console.error('批量加载图片失败：', e)
        this.error = '部分图片加载失败，请刷新重试'
      }
    },
    //提取文件名
    getFileNameFromUrl(url) {
      ...
      return pathname.split('/').pop().split('?')[0] || `image_${Date.now()}`
    },
    //获取店铺信息
    async GetShopInfo() {
      try {
    ...
    async submit() {
      ...
      try {
        ...
        if (response.code == 1) {
          const imgResponse = await uploadImage(this.id, 'post', this.ImagesFiles)
