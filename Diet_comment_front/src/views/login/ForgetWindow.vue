<template>
  <div class="page">
    <form class="window" @submit.prevent="submitPassword">
      ...
      <div class="confirm">
        ...
        <button @click="$router.push('/login')" class="confirm-btn">返回</button>
      </div>
      <div>
...
export default {
  ...
  methods: {
    ...
    async submitPassword() {
      ...
      try {
        const response = await findPasswordApi(this.username, this.password, this.email, this.code)
        if (response.code === 1) {
          this.success = '密码修改成功，正在跳转到登录页面...'
          this.error = ''
          setTimeout(() => {
        ...
        this.success = ''
        this.error = error.message || '修改失败，请稍后重试'
      }
    },
  },
}
